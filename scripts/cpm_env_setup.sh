...
#    update_progress "env_setup_completed"
    ...
    # Conda sucks, it conflicts with bash, we can't actually deactivate 
    # it in a script. So we break here and require the user to do it
    # in their terminal session.
    # we can not find conda.sh without guess as relative to the path
    # returned by which conda
    # Install CPM Andes Golden Model
#    safe_export_path "${RV_ANDES_GNU_BAREMETAL_TOOLS}/bin"
#    
#    if ! check_progress "cpm_andes_spike_installed"; then
#        echo_stage "Building and Installing CPM Andes Spike"
#        cd "${CONDOR_TOP}"
#        if ! bash how-to/scripts/build_cpm_andes_spike.sh; then
#            pretty_error "Failed to install CPM Andes Spike. See log: ${LOG_FILE}"
#            exit 1
#        fi
...
#
#    echo
#    echo "CPM environment setup completed successfully."
}
...
#        printf "  %-25s : %s\n" "PATCHES" "${PATCHES:-<undefined>}"
#        printf "  %-25s : %s\n" "RV_GNU_LINUX_TOOLS" \
#               "${RV_GNU_LINUX_TOOLS:-<undefined>}"
#        printf "  %-25s : %s\n" "RV_GNU_BAREMETAL_TOOLS" \
#               "${RV_GNU_BAREMETAL_TOOLS:-<undefined>}"
#        printf "  %-25s : %s\n" "RV_ANDES_GNU_BAREMETAL_TOOLS" \
#               "${RV_ANDES_GNU_BAREMETAL_TOOLS:-<undefined>}"
#        printf "  %-25s : %s\n" "CPM_ANDES_SPIKE_DIR" \
#               "${CPM_ANDES_SPIKE_DIR:-<undefined>}"
#        echo
#        echo "To set these variables, cd to your work area and run:"
#        echo "  source how-to/env/setuprc.sh"
#        exit 1
