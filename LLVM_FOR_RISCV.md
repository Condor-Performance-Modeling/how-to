...
#### Generate `RISCVGenMacroFusion.inc`

Macro Fusion Predicators live in `RISCVGenMacroFusion.inc` file that is generated by default during LLVM make.

- `RISCVGenMacroFusion.inc` can be generated by making the `RISCVCommonTableGen` target in the build directory:
...
- Alternatively you can generate it by running the following command:

  ```bash
  cd [PATH_TO_YOUR_LLVM_BUILD_DIR]/lib/Target/RISCV
  ../../../bin/llvm-tblgen -gen-macro-fusion-pred \
  -I [PATH_TO_YOUR_LLVM_INSTALL]/llvm/lib/Target/RISCV \
  -I [PATH_TO_YOUR_LLVM_BUILD_DIR]/include \
  -I [PATH_TO_YOUR_LLVM_SOURCE]/llvm/include \
  -I [PATH_TO_YOUR_LLVM_SOURCE]llvm/lib/Target \
  [PATH_TO_YOUR_LLVM_SOURCE]/llvm/lib/Target/RISCV/RISCV.td \
  --write-if-changed -o [PATH_TO_YOUR_LLVM_BUILD_DIR]/lib/Target/RISCV/RISCVGenMacroFusion.inc
  ```

This will add your fusion predicator to `RISCVGenMacroFusion.inc` and allow to use it in processor definition.
...
Change to `RISCVProcessors.td` is reflected in `RISCVTargetParserDef.inc` and `RISCVGenSubtargetInfo.inc`. To generate those files make the `RISCVTargetParserTableGen` and then `RISCVCommonTableGen` target in the build directory:
...
After generating the required files, you need to install the updated LLVM build to apply the changes.

1. **Install the Updated LLVM Build**:
   Run the `make install` command to install the changes:
...
2. **Verify the Processor Configuration**: 
   ...
   ```bash
   [PATH_TO_YOUR_LLVM_INSTALL]/bin/clang -target riscv64 -mcpu=help
   ```

3. **Compile with the New Processor**:
   To compile code using your custom processor definition, use the `-mcpu` or `-mtune` option with LLVM tools (example):

   ```bash
   [PATH_TO_YOUR_LLVM_INSTALL]/bin/clang -target riscv64 -mcpu=my-custom-processor -o output.o input.c
   ```
